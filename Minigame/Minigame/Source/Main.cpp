...
void Start()
{
	...
	state.ship2 = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/enemy.png"));
	state.shot = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/shot.png"));
	state.shot2 = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/shot2.png"));
	state.explosion = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/explosion.png"));
...
void MoveStuff()
{
	...
	if (state.keyboard[SDL_SCANCODE_P] == KEY_DOWN && state.ship2_life != 0)
	{
		...
		state.shots2[state.last_shot2].x = state.ship2_x - 35;
		state.shots2[state.last_shot2].y = state.ship2_y + 3;
		state.last_shot2++;
...
void Draw()
{
	...
	rec.x = state.ship2_x; rec.y = state.ship2_y; rec.w = 64; rec.h = 64;
